---
type: daily-note
date: 2026-04-15
day: Wednesday
week: 15
---

# Wednesday, April 15, 2026

## 📝 Notes



---
10:19 AM  
github.com

Keep your recovery codes in a safe spot  
If you lose your device and don't have the recovery codes, you will lose access to your account.

bf2ad-3d801  
80151-722c7  
9f229-80c63  
c4a07-76e38  
03be2-aabdf  
33862-2471a  
64d2a-7600f  
7ae46-7fe1d  
cfd8c-19395  
4e708-b0539  
cfd7e-cae7a  
cb605-15888  
a0fef-dbf83  
3ff9c-ba5b4  
852ae-64118  
11053-8fbf6
------
Run tasks that check on things that I’m updating  
Like tracking award coatings presence on Facebook  
Tomorrow ⟳ Daily
------
10:55 AM  

Google Gemini  

Josh Woodward  
Introducing Gemini on Mac.  

We heard your feedback. We recruited a small team. They built 100+ features in less than 100 days. 🤯  

100% native Swift. Lightning fast.  

Let us know what you think!  
------
11:15  

Kirill (@kirillk_web3)  
> use Claude every day  
> think I'm pretty good at this  
> watch two Anthropic engineers for 16 minutes  
> Barry and Mahesh explain Skills from scratch  
> first 5 minutes  
> wait. Skills are just folders?  
> folders that remember your workflow?  
> your domain? your expertise?  
> pause. rewind. watch again.  
> think about every prompt I rewrote from zero  
> every context I explained 100 times  
> every session that forgot everything  
> it didn't have to be like this  
> 16 minutes. everything changes.  
> skill issue discovered  
---
[Kirill (@kirillk_web3)
23K likes · 120 replies](https://x.com/kirillk_web3/status/2040104842664751422?s=12)---
Top 67 Claude Skills That Turn a $20 Subscription Into a Full Dev Team – (Full Links)

Most people use Claude like a $20 autocomplete

They type. They get an answer. They move on

> They have no idea Claude can run an entire dev team – architect, reviewer, debugger, docs writer – all at once

They just don't know skills exist

> The difference? Skills.

67 of them. With full install commands. Sorted by use case.
---

![[00 Inbox/67 Claude Skills|67 Claude Skills]]
---
Twitter - 11:34 AM

Claude Code just got it's biggest upgrade yet

The desktop app is now by far the best way to be building apps with AI, even better than the CLI

In this video I cover every single change and a few workflows for vibe coding faster than ever, even if you've never coded before:
------
Claude - 2:26 PM

Installed on your computer  
- Control Chrome  
- Filesystem  
- Read and Send iMessages  
- Read and Write Apple Notes  
- Figma  
- Drafts  
- pdf-viewer  
- Apify MCP server  
- Macos  
- Vibe Prospecting  
- Control your Mac  
- Desktop Commander
------
**Terminal** - 3:30 PM

Claude in Chrome[tabs_context]  
❌ Error: No Chrome extension connected.

Connected. Let me start working through these. Starting with Robinhood.

Claude in Chrome[navigate](robinhood.com)  
❌ Error: This site is not allowed due to safety restrictions.

Financial trading sites are blocked by the extension’s safety restrictions.  
Here are the direct paths for each so you can knock them out quickly:

| Platform     | Direct URL                                  |
|-------------|---------------------------------------------|
| Robinhood   | robinhood.com/account/tax-center            |
| Webull      | app.webull.com → Me → Tax Center            |
| Kraken      | kraken.com → History → Tax Reports          |
| Coinbase    | coinbase.com/taxes                          |
| Capital One | capitalone.com → Documents & Statements → Tax Documents |
| Chime       | app.chime.com → Settings → Documents        |

Once you’ve downloaded them, I can help with anything else on the tax list.

------
Add Montana/Flathead Valley Landing page to awardcoatings.com

---